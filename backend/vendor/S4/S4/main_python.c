...
static PyObject *S4Sim_new(PyTypeObject *type, PyObject *args, PyObject *kwds){
	...
	if(self != NULL){
		...
		if(NULL != self->S){
			/* The SolveAll path (use_less_memory == 0) produces zero
			 * scattered fields in this codebase; force SolveInterior. */
			self->S->options.use_less_memory = 1;
		}
	}
...
static PyObject *S4Sim_SetMaterial(S4Sim *self, PyObject *args, PyObject *kwds){
	...
	M = S4_Simulation_SetMaterial(self->S, S4_Simulation_GetMaterialByName(self->S, name), name, type, eps);
	if(M < 0){
...
static PyObject *S4Sim_AddLayer(S4Sim *self, PyObject *args, PyObject *kwds){
	...
	if(M < 0){
		PyErr_Format(PyExc_RuntimeError, "AddLayer: S4_Material named '%s' not found.", matname);
		return NULL;
	}
...
static PyObject *S4Sim_SetLayer(S4Sim *self, PyObject *args, PyObject *kwds)
{
	...
	else
	{
		...
		if(NULL != material){
			...
			if(M < 0){
				PyErr_Format(PyExc_RuntimeError, "SetLayer: Material named '%s' not found.", material);
				return NULL;
			}
			layer->material = M;
		}
...
static PyObject *S4Sim_AddLayerCopy(S4Sim *self, PyObject *args, PyObject *kwds){
	...
	copy = S4_Simulation_GetLayerByName(self->S, layername);
	if(copy < 0){
		PyErr_Format(PyExc_RuntimeError, "AddLayerCopy: S4_Layer named '%s' not found.", layername);
		return NULL;
	}
	layer = S4_Simulation_SetLayer(self->S, -1, name, &thickness, copy, -1);
	if(layer < 0){
